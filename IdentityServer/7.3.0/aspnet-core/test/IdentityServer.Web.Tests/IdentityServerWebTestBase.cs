...
{
    public abstract class IdentityServerWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static IdentityServerWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(IdentityServerWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<IdentityServerDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<IdentityServerDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<IdentityServerDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<IdentityServerDbContext, Task<T>> func)
        {
            T result;
